...
use app\student\model\View;
use app\student\model\Fankui;
use app\student\model\Fan;
use app\student\model\Perform;
...
class Index extends Base
{
    ...
    $this ->view ->assign('name',Session::get('xm'));
...
$bb = User::query('
       ...
       WHERE
           ...
           //$bbb = $perform ->where('number',$a) ->where('year',$year) ->where('date',$v[$i]['yue']) ->select();
...
//return dump($pic[0]['sum']);
    ...
    return $this ->view ->fetch('view/one');
     ...
     }else
     {
	    ...
	    public function fankuia()
	   {
	     ...
	     return $this ->view ->fetch('loadimg');
}
    ...
    $this ->success('上传成功,正在返回......','index/index');  
      ...
      public function chulii(){
        $this -> isLogin();
     $fankui = new Fankui;
     $fan = $fankui ->where('stat',1) ->where('student_id',Session::get('xh'))->select();
     $arr = array_reverse($fan);
    $this ->view ->assign('fan',$arr);
    return $this ->view ->fetch('chuli');  
      //return dump($fan);
     }
...
public function viewgrade2(){
  ...
  return dump($grade);
     ...
     // public function stuok(){
     // $perform = new Perform;
     // $view = new View;
     // $yue = $view -> where('status',1) -> find();
     // $xh = Session::get('xh');
     ...
     foreach ($yue as $key => $value) {
       ...
       $xh = Session::get('xh');
       //$xh = 12165108;
       $perform ->where('number',$xh) ->where('date',$a) ->where('year',$year) ->update(['status'=>1]);

     }
     $this ->success('您已经确认成绩！请知晓！');
     //return "<script>alert(\"您已经确认成绩！请知晓！\");</script>";
     }

// public function gai(){
//     $student = new Student;
//     $student ->where('political_status','中国共产主义青年团团员') ->update(['political_status'=>'共青团员']);
//     return 'OK';
// }
